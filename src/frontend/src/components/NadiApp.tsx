...
interface NadiAppProps {
  onBackToHome?: () => void;
  onGoToNumerology?: () => void;
}

export default function NadiApp({
  onBackToHome,
  onGoToNumerology,
}: NadiAppProps = {}) {
  ...
  const getPickPrompt = (ps: PickState): string => {
    ...
    if (ps.phase === "pick-blue") {
      ...
      if (ps.blueTarget === 3 && ps.bluePickedCount === 0) {
        return `${rowLabel}: Tap up to 3 Blue cards`;
      }
    ...
    setTimeout(() => {
      ...
      setPickState((prev) => {
        if (prev.phase === "pick-red") {
          ...
          const blueTarget = getBlueCount(redCard.planet);
          return {
            ...prev,
            phase: "pick-blue",
            blueTarget,
        ...
        if (prev.phase === "pick-blue") {
          ...
          if (prev.blueTarget === "repeat") {
            const wasRepeat = prev.blueHousesSeen.includes(blueCard.name);
            if (wasRepeat) return advanceRow(prev, newSeen);
            return {
              ...prev,
  ...
  return (
    <div className="min-h-screen" style={{ background: "#e8e8e8" }}>
      ...
      <div style={{ background: "#f5a623" }}>
        ...
        <div className="flex items-center justify-between px-3 py-2">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-white text-base">Nadi Cards</span>
          </div>
          <div className="flex gap-2 items-center">
            {onGoToNumerology && (
              <button
                type="button"
                onClick={onGoToNumerology}
                  ...
                  >
                    {spreadBlue
                      ...
                      .map((sc) => {
                        ...
                        return (
                          <SpreadCardItem
                            key={(sc.card as BlueCard).id ?? `blue-${origIdx}`}
                            sc={sc}
                            cardType="blue"
                            index={origIdx}
                  ...
                  >
                    {spreadRed
                      ...
                      .map((sc) => {
                        ...
                        return (
                          <SpreadCardItem
                            key={(sc.card as RedCard).id ?? `red-${origIdx}`}
                            sc={sc}
                            cardType="red"
                            index={origIdx}
                    ...
                    {spreadYellow
                      ...
                      .map((sc) => {
                        ...
                        return (
                          <SpreadCardItem
                            key={
                              (sc.card as YellowCard).id ?? `yellow-${origIdx}`
                            }
                            sc={sc}
                            cardType="yellow"
                            index={origIdx}
      ...
      {/* Reading area */}
      <div className="px-3 pt-4 pb-6" style={{ background: "#e0e0e0" }}>
        {mode === "auto" && (
          <>
            {rows.length === 0 && (
              <div
